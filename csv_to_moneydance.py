    ...
    with open(csv_file, 'r', encoding='utf-8-sig') as csvf:
        ...
        for idx, row in enumerate(reader, start=1):
            try:
                ...
                # if they have a 0 amount associated with them. These are effectively "empty" transactions.
                if transaction != {} and not (transaction['symbol'] == '315994103' and transaction['amount'] == 0):
        ...
        script_lines.append("")
        script_lines.append("from com.infinitekind.moneydance.model import AccountUtil, ParentTxn, CurrencyType, InvestFields, InvestTxnType")
        ...
        script_lines.append("")
        script_lines.append("def findAccount(name):")
        script_lines.append("    \"\"\"Function to find account by name\"\"\"")
        script_lines.append("    account = None")
        script_lines.append("    for acct in root.getRootAccount().getSubAccounts():")
        script_lines.append("        if acct.getAccountName() == name:")
        script_lines.append("            return(acct)")
        script_lines.append("        for subacct in acct.getSubAccounts():")
        script_lines.append("            if subacct.getAccountName() == name:")
        script_lines.append("                return(subacct)")
        script_lines.append("")
        script_lines.append("def findSecurity(ticker):")
        script_lines.append("    \"\"\"")
        script_lines.append("    Function to find security with either a matching ticker symbol or a mathching CUSIP.")
        script_lines.append("    Returns a list of all securities with a matching ticker symbol if there are any matches.")
        script_lines.append("    If no matches are found, then the function interprets the argument as a CUSIP instead.")
        script_lines.append("    \"\"\"")
        ...
        script_lines.append("    \"\"\"")
        script_lines.append("    Function to find subaccount associated with a security")
        script_lines.append("    Returns a list of all subaccounts with a matching ticker symbol.")
        script_lines.append("    If there are matches, then only aubaccounts with a non-zero balance are returned.")
        script_lines.append("    This is because if a new security with the same ticker is created due to a change in CUSIP,")
        script_lines.append("    we only want the new security. This can happen with mergers, for example.")
        script_lines.append("    \"\"\"")
        ...
        script_lines.append("account = findAccount(\"" + escape_string(account_name) + "\")")
